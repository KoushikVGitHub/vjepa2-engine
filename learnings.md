...
**L19 — Phase 2b/2c (batch-96 confirmation + reviewer-control battery) IN PROGRESS 2026-07-29 — PARTIAL: linear/mlp/H3/H5 + S1 done; conv/convdisjoint/H9/S2 still training on a 32 GB pod. Early signal REVISES L18 against interest: capacity/nonlinearity — not overlap — explains much of the tokenizer gain, the corrected pk floor is HIGHER, and the transformer LOSES in-suite information.**
...
- **PENDING (32 GB pod, antigravity agent, shared `/workspace`):** conv @b96 (does 0.766 survive at rank 35 vs linear's new 0.638?), **convdisjoint (S3 overlap decider)**, H7 random-init floor, H9 strict-hygiene, S2 pretraining-seed spread. Verdict + final table computed by `scripts/phase2_verdict.py` (auto-resolves the read rules) once these land.
...
- **Net so far (provisional):** the headline is shifting from "conv-stem band-limiting win" to a more honest, more interesting story — **in-suite, SSL is below its own raw tokenizer and below pk; the value proposition rests entirely on cross-suite *transfer*.** Finalize L19 once conv/convdisjoint/H9/S2 report.
