...
**L22 — DOF-grounded small-encoder campaign on RTX 4090 (2026-08-06) — VERDICT: right-sizing bought "harmless, not helpful". The conv tokenizer does the work; SSL ties but never beats it, in-suite AND on transfer. Honest null on the DOF hypothesis; both invariance levers null too.**
- **Setup & why.** Single RTX 4090 24 GB. Tests whether right-sizing the encoder to the data's measured DOF (L13: task lives in ~10–50 dims, not 1024) + dropping the EMA teacher for a symmetric objective finally yields *transferable* cosmology. All arms: patch-8, conv stem, edim-64, 4 layers, 4 heads, 4000 steps, batch-64 `--ckpt`, no EMA / no stop-grad. **4.2 M params** (~70× smaller than the L21 ViT-L).
- **Operating point was measured, not assumed.** patch-8's 1024-token attention is O(N²·batch); a 24 GB card OOMs at batch-96 (that rank regime needs the 2-GPU distributed-SIGReg path), so batch-64 `--ckpt` is the ceiling. Activation checkpointing is MANDATORY even for a tiny model — activations, not params, are the bottleneck. eff_rank plateaus ~35 (= 0.55×64), above the task DOF k95≈10. RANK-BOUND vs the A100 batch-96 numbers, but self-consistent across every arm here.
- **Loss-mode clarification (a conflation was caught).** `LOSS_MODES = ['ema','lejepa','visreg']` are three distinct objectives. `lejepa` = SIGReg (characteristic-function / Cramér–Wold isotropy). `visreg` = sliced-Wasserstein-to-Gaussian + optional VICReg var/cov (`--visreg-lambda` 0.8). Both lejepa AND visreg are no-EMA / no-stop-grad symmetric (shared encoder run twice, no teacher); `ema` is the stop-grad-teacher control.
- **Noise band FIRST (task 0b) — the S2 dispute is CLOSED.** 5 pretraining seeds of the anchor lejepa arm, probed in-suite (ITNG Mgas, attentive-MLP head): **Ω_m 0.866 ± 0.017, σ8 0.566 ± 0.025**. So the true pretraining-seed σ ≈ **0.017 / 0.025** — NOT the 0.115 the reviewer feared (that was the parked antigravity outlier). Transfer r²aff seed-σ is larger, ~0.05–0.07. Replicate prescription: ~5 seeds resolve a 0.03 Ω_m in-suite effect; a 0.06 *transfer* effect needs ~12 (MDE).
- **The gate — and it does NOT clear.** Floors on the anchor geometry (seed-0, in-suite MLP): raw-tokenizer (conv stem, `--probe-stage tokenizer`) Ω_m **0.870** / σ8 0.558; random-init Ω_m 0.819 / σ8 0.516. The trained lejepa encoder (0.866 / 0.566) **ties its own raw tokenizer** (Ω_m −0.004, σ8 +0.008, both within σ). ⇒ the in-suite "beats ViT-L (0.809) and pk foil (0.834)" reading is a **TOKENIZER win, not an SSL win.** Nuance vs L21: the tiny transformer is NON-DESTRUCTIVE in-suite (ties the tokenizer) where the ViT-L was destructive (0.809 « 0.870). DOF-sizing bought "harmless", not "helpful".
- **Transfer (ridge r²aff — the readout that matters; raw R² negative for everything, nothing deployable).** lejepa n=5: Ω_m **0.159 ± 0.068**, σ8 0.252 ± 0.027. visreg n=5: Ω_m **0.201 ± 0.051**, σ8 0.257 ± 0.030 (visreg in-suite MLP Ω_m 0.888 ± 0.016 / σ8 0.576 ± 0.035). Floors (seed-0): raw-tok Ω_m 0.130 / σ8 0.195; random-init Ω_m 0.154 / σ8 0.210.
- **Corrected χ² gate verdict (n=5): everything INDECISIVE vs the tokenizer floor.** visreg has the BEST point estimates on every axis (visreg > lejepa > floor, consistently), but nothing is statistically resolved: visreg transfer Ω_m Δ+0.071 (UCB band 0.120); σ8 Δ+0.062 (band 0.072); in-suite Δ+0.018 (band 0.038); visreg-vs-lejepa Δ+0.042. The ONLY decisive lines are vs random-init. ⇒ **both objectives beat random-init but NEITHER beats its own conv stem; the tokenizer does the work.** Objective-level effects (~0.04–0.07 r²aff) are sub-noise at n=5. (⚠ "prefer visreg over lejepa" is directionally supported by point estimates but NOT proven — do not claim visreg > lejepa.)
- **Invariance levers — both NULL for transfer.** Suite-de-classification (DANN GRL + suite adversary) and feedback-parameter invariance (shed A_SN/A_AGN via GRL): the naive suite flip died because ITNG-vs-Astrid separability is 0.512 (chance) — no adversary work to do. Pivoted to feedback-invariance on a truly-held-out SIMBA; GRL effect λ1−λ0 = −0.014 (n=2). The de-confounding lever does not move cross-suite transfer here.
- **One open exploratory lead (n=2, UNCONFIRMED).** Trained Mgas-only SSL (~0.21) beat *both* floors on held-out SIMBA Ω_m at n=2 (rawtok 0.062, randinit 0.154) — i.e. the *training* helping, not the invariance; single-field (probe-matched) beats L21's 12-field null. Was being confirmed at n=5 when the campaign paused (`run_simba_experiment.sh "0 1 2 3 4" feedback_plain` → `simba_verdict.py`). Do NOT quote it as a result yet.
- **Two corrections on the record (the framework catching over-reads).** (1) The in-suite "small beats big" win is the tokenizer, not the SSL. (2) Seed-0's transfer r²aff of 0.270 looked like a doubling over the floor; the 5-seed band collapsed it to 0.159 ± 0.068 = indistinguishable from the floor. The noise band caught the n=1 over-read — exactly its job.
- **Tooling built.** `scripts/ridge_readout.py` (pure-numpy ridge, no sklearn; mean-pool frozen features; reuses `run_probe._feat_sig` for exact cache-key match — MUST include `stem_pad`; cross-suite split named `heldout` not `test`; reports in-suite R² + transfer raw R² + r²aff; self-tested). `src/declassify.py` (GRL + suite/feedback adversaries, self-tested: GRL erases suite 0.55→chance, feedback control 0.65→−0.01). `eval/claims/astrid_transfer.jsonl` (hash-chained pre-registration — the GRL claims are the pre-reg; the Mgas-only lead is post-hoc/exploratory).
- **Infra footnotes.** RunPod 4090 = `runpod_auto` key (`id_ed25519` NOT authorized). tmux/apt unavailable (no network) → detach via `setsid`. `eval/lib/gate.py` imports scipy → judge runs where scipy exists (now pip-installed on the pod). `_probe_feat_cache` is the recurring volume-filler (93 GB) — re-derivable, delete freely.
- **VERDICT & next.** DOF-sizing makes the encoder *harmless* in-suite (ties tokenizer, vs ViT-L's destruction) but does NOT crack transfer — L21's honest null stands. Objective tuning and architecture sweeps are sub-noise, an unaffordable way to chase a small gain. The only lead with a plausibly-resolvable effect is single-field trained SSL on the hard suite (the n=5 confirmation); L21's ranked fixes (multi-suite / SIMBA-only pretraining, label-aligned *alignment* not just isotropy) remain the real work. **Do not ship "the JEPA learned transferable physics."**
