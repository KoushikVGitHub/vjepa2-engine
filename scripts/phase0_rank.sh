...
torchrun --standalone --nproc_per_node=2 src/train_fsdp.py $C --save $WS/ckpt_p8_b128.pt > $WS/p0_train.log 2>&1
echo "train rc=$? ($(grep -c '^step' $WS/p0_train.log 2>/dev/null) step-logs)"

ck() { if [ "$1" = "$STEPS" ]; then echo "$WS/ckpt_p8_b128.pt"; else echo "$WS/ckpt_p8_b128_step$1.pt"; fi; }
pr() { CUDA_VISIBLE_DEVICES=$2 python scripts/run_probe.py --ckpt "$(ck $1)" --field Mgas \
  --img 256 --patch 8 --enc-d 1024 --enc-layers 24 --enc-heads 16 --no-atlas --seed $SEED > "$WS/p0_pr_$1.log" 2>&1; }
...
for S in $PROBE_STEPS; do
  R=$(grep -A2 "IN-SUITE" "$WS/p0_pr_$S.log" 2>/dev/null | grep "R2" | grep -oE "Omega_m=[0-9.]+ +sigma8=[0-9.]+")
  printf "%-8s | %-30s\n" "$S" "$R"
done
