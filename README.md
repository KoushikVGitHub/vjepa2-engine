...
scripts/        # production drivers
  ...
  analyze_all.py  per-field statistics -> curation thresholds
tests/          # CPU correctness gates, run in CI on every push (see Correctness gates)
```

The from-scratch fundamentals — the collapse studies, the SIGReg/VICReg demos, and the day-by-day notes — now live in a companion repo, **[vjepa-study](https://github.com/KoushikVGitHub/vjepa-study)**, to keep this repo focused on the experiment. That study code imports this engine's `src/`, never the reverse.
...
# https://github.com/KoushikVGitHub/vjepa-study/blob/main/notes/collapse_resolution.md.
#
...
- [x] **Dimensional collapse diagnosed and fixed:** effective rank **2 → 38.7**, target-std → 0.96. Root cause = SIGReg's marginal test is near-blind to anisotropic collapse (‖∇‖ ≈ 2e-4 vs 1.25 for a covariance penalty); fix = VICReg var/cov at a var-dominant ~125:1 ratio + target LayerNorm. Three failed gates isolated **var = the scale knob, cov = the rank knob**. [`https://github.com/KoushikVGitHub/vjepa-study/blob/main/notes/collapse_resolution.md`](https://github.com/KoushikVGitHub/vjepa-study/blob/main/notes/collapse_resolution.md).
...
- [x] **Inference optimization (Stage 4):** latency p50/p99 + throughput + MFU per lever (`https://github.com/KoushikVGitHub/vjepa-study/blob/main/notes/day5_inference.md`).
